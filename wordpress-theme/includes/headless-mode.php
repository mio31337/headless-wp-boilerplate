 ...
 */
add_action( 'admin_menu', 'wp_headless_mode_settings' );
function wp_headless_mode_settings() {
    add_submenu_page(
        'options-general.php',
        ...
        'manage_options',
        'headless-mode',
        'wp_headless_mode_settings_output'
    );
}

function wp_headless_mode_settings_output() {
    $clientUrl = HEADLESS_MODE_CLIENT_URL !== 'https://example.com' ? HEADLESS_MODE_CLIENT_URL : false;
    ?>
    <div class="wrap">
        <h2>
 ...
 */
function wp_headless_mode_redirect($url, $permanent = false) {
 ...
 */
add_action( 'parse_request', 'wp_headless_mode_disable_front_end', 99 );
function wp_headless_mode_disable_front_end() {
    /**
    ...
    if (
        ...
        HEADLESS_MODE_CLIENT_URL !== 'https://example.com' &&
        ! is_admin() &&
        (
        ...
        if( apply_filters( 'headless_mode_will_redirect', true, $new_url ) ){
            wp_headless_mode_redirect( $new_url, true );
            exit;
        }
    }
}